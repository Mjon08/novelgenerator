...
function initKeyboardShortcuts() {
  document.addEventListener('keydown', (e) => {
    ...
    if (mod) {
      if (key === 's' || code === 'KeyS') { e.preventDefault(); simpanManual(); }
      else if (key === 'f' || code === 'KeyF') { e.preventDefault(); toggleFindReplace(); }
      else if (key === 'b' || code === 'KeyB') { e.preventDefault(); quill.format('bold', !quill.getFormat().bold); }
      else if (key === 'i' || code === 'KeyI') { e.preventDefault(); quill.format('italic', !quill.getFormat().italic); }
      else if (key === 'u' || code === 'KeyU') { e.preventDefault(); quill.format('underline', !quill.getFormat().underline); }
    }
    if (e.key === 'F11' || e.code === 'F11') { e.preventDefault(); toggleFocusMode(); }
    if (e.key === 'Escape') {
...
function navigasiBab(bab) {
  ...
  // Di mode laci, panel menutupi kanvas — tutup agar bab yang dipilih terlihat.
  if (typeof modeLaci === 'function' && modeLaci()) tutupLaciSidebar();
}
...
// Di bawah lebar ini panel bab tampil sebagai laci mengambang, bukan kolom
// tetap, agar tidak memakan ruang tulis. Nilainya harus sama dengan
// breakpoint pada CSS editor.html.
const LEBAR_MODE_LACI = 900;

function modeLaci() {
  return window.matchMedia(`(max-width: ${LEBAR_MODE_LACI}px)`).matches;
}
...
function toggleSidebar() {
  ...
  if (modeLaci()) {
    const terbuka = sidebar.classList.toggle('drawer-open');
    backdrop?.classList.toggle('visible', terbuka);
    return;
  }
...
function tutupLaciSidebar() {
  document.getElementById('sidebar')?.classList.remove('drawer-open');
  document.getElementById('sidebar-backdrop')?.classList.remove('visible');
}

// Saat layar dilebarkan kembali ke ukuran desktop, sisa state laci harus
// dibersihkan agar panel tidak tertinggal tersembunyi.
